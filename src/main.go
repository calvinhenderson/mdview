...
func renderFile(path string) {
    ...
    // Uses github api to parse markdown
    ...
    rendered := template.JSEscapeString(string(html))

    wview.Dispatch(func() {
        wview.Eval(fmt.Sprintf(`
            document.getElementsByClassName("markdown-body")[0].innerHTML = "%s";
        `, rendered))
    })
}
...
func main() {
    if len(os.Args) != 2 {
        log.Fatal("usage: mdview <file>")
    }

    file, _ := filepath.Abs(os.Args[1])
    ...
    wview = webview.New(webview.Settings{
    ...
    wview.Dispatch(func() {
        css := `
            ...
            // Courtesy of https://github.com/sindresorhus/github-markdown-css
        ...
        wview.Eval(fmt.Sprintf(`
            document.getElementById("app").innerHTML = "<div class='markdown-body'></div>";
            var head = document.getElementsByTagName("head")[0];
            head.innerHTML += "<style>%s</style>";
